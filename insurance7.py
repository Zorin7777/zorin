...
def introduce_page():
    ...
    st.markdown(
        """
        # 医疗费用预测应用💰
        这个应用利用机器学习模式来预测医疗费用，为保险公司的保险定价提供参考。
        ## 背景介绍
        - 开发目标：帮助保险公司合理定价保险产品，控制风险。
        - 模型算法：利用随机森林回归算法训练医疗费用预测模型。
        ## 使用指南
        - 输入准确完整地被保险人信息，可以得到更准确地费用预测。
        - 预测结果可以作为保险定价地重要参考，但需审慎决策。
        - 有任何问题欢迎联系我们的技术支持。
        
        技术支持：1011145988@qq.com
""")

    
def predict_page():
    """当选择预测费用界面时，将呈现该函数的内容"""
...
""")
    ...
    st.write("技术支持：1011145988@qq.com")
st.set_page_config(
    page_title="医疗费用预测",
    page_icon="💰",
)
